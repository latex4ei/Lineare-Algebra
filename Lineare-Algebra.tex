...
Dimensionsformel: $\rang(A) + \mathrm{dim}(\ker(A)) = n$ \\
...
Die Lösung des LGS $(A|b)$ hat $\dim(\ker A) = n-\rang(A)$ frei wählbare Parameter.\\
\\
...
\begin{tabular}{ll}	
	1)  $A$ ist invertierbar & 2)  $\rang(A)=n$ \\
	3)  $\mathrm{Kern}(A)={0}$ & 4) $\dim(\col(A))=\dim(\row(A))=n$\\
...
\begin{itemize}\itemsep0pt
\item $\ker(f) = \{ x\in\mathbb{R}^n : f(x) = 0 \}$
\item $\Bild(f) = \col([f])$
\item $f$ injektiv, wenn $\ker([f])=\{0\}$
\item $f$ surjektiv, wenn $\Bild([f])=\mathbb{R}^m$
...
\begin{equation*}
V_i=\ker(A-\lambda_i\mathbb{I}_n)=\text{span}(B_i)
\end{equation*}
