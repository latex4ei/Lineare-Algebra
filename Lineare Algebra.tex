...
\fancyhf{}
   ...
   \fancyfoot[C]{\textbf{Lineare Algebra} von Lukas Kompatscher (lukas.kompatscher@tum.de)}
...
\end{itemize}
\textbf{Matrix $A$ heißt orthogonal}, wenn $A^\top A = E_n$
\begin{itemize}\itemsep0pt
\item $A^{-1}=A^\top $
\item $\det{A}=\pm1$
...
e^{a_1t} & \ldots & 0 \\ 
\vdots & \ddots & \vdots \\ 
0 & \ldots & e^{a_nt} \end{pmatrix} = \diag\enbrace{e^{a_1t},\dots,e^{a_nt}}
\end{equation*}
...
\begin{equation*}
e^{tA}=Ve^{tD}V^{-1}=
V\diag\enbrace{e^{\lambda_1t},\dots,e^{\lambda_nt}}V^{-1}
\end{equation*}
\textbf{Jordanblock}
$A=\begin{pmatrix}
a_1 & 1\\
...
0& 0& a_3
\end{pmatrix}$\\
Aufteilen von $A$ in $A=D+N=\begin{pmatrix}
a_1 & 0\\
0 & a_2
...
\begin{equation*}
\begin{split}
\Rightarrow e^{At}&=e^{Dt+Nt}=e^{Dt}e^{Nt}\\
&=\diag\enbrace{e^{a_1t},\dots,e^{a_nt}}\enbrace{E_n+tN+t^2\frac{N^2}{2}+\dots}
\end{split}
\end{equation*}\\
Die Potenzreihe von $e^{Nt}$ ist auf endliche Summanden begrenzt.
\end{multicols*}
% Ende der Spalten
